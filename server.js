...
//  this is the where i build the logic of saving the files
const myStorageSettings = multer.diskStorage({
    // i use destination: request file callback to tell the server the file should always go in the uploads folder
    destination: function (request, file, callback) {
        callback(null, 'uploads/');
    },
    // to keep it simple, i tell the server to name the file drawing.png - this allows for context
    filename: function (request, file, callback) {
        callback(null, 'drawing.png');
    }
});

// i am telling multer to use the logic i defined in the above block
const upload = multer({ storage: myStorageSettings });
